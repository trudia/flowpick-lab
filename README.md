...
## v22-natural-copy-update-2026-05-22

v22 자연스러운 전환형 글쓰기 업데이트:
- PASONA 용어를 공개 글에서 제거
- PASONA는 내부 글쓰기 구조로만 유지
- 글 내부의 `P 문제`, `A 공감` 같은 마케팅 템플릿 느낌 제거
- 자연스러운 섹션: 이런 문제가 있다면 / 처음부터 완벽하게 만들려고 하지 마세요 / 먼저 이렇게 시작하세요
- 홈페이지와 CTA 문구를 `~합니다` 설명형에서 `~하세요` 안내형으로 개선
- 구매 전 비교 박스를 더 행동 유도형 문장으로 수정
- editor-guide에는 “공개 글에는 PASONA 용어를 직접 쓰지 말라”는 운영자 메모 추가
