...
## v23-natural-conversion-final-2026-05-22
- 공개 페이지에서 PASONA/P-A-S-O-N-A 용어 제거
- 글 내부 흐름을 자연스러운 문제 해결형 문장으로 재구성
- 홈페이지 문구를 ~하세요 안내형으로 정리
- 검색 페이지 빠른 키워드 버튼을 확실히 노출
- 구매 박스를 추천 대상/먼저 볼 항목/사지 않아도 되는 경우/먼저 해볼 것 구조로 변경
- 제휴 안내 문구를 사용자 친화적으로 수정


## v24-visitor-copy-cleanup-2026-05-23

v24 방문자용 문구 정리:
- 카테고리의 운영자용 문구 제거
- 글 내부 `Guide Flow`, `Affiliate Ready` 표현을 자연스러운 문구로 변경
- 제휴 안내 페이지에서 data/json/editor 등 운영자용 문구 제거
- 구매 박스를 `먼저 볼 항목 / 이런 사람에게 좋아요 / 사지 않아도 되는 경우 / 먼저 해볼 것` 구조로 변경
- 검색 키워드 버튼을 더 눈에 띄게 개선
- 홈 하단 설명 문구를 짧게 축약


## 전/후 이미지 운영 원칙
- 전/후 변화형 글은 한 장의 비교 이미지(정리 전 / 정리 후)를 함께 넣습니다.
- 비교형 글은 좌우 비교 또는 2~3분할 비교 이미지를 우선 사용합니다.
- 같은 이미지를 여러 글에 반복하기보다, 주제별 비교 이미지를 따로 두는 편이 좋습니다.


## 이미지 운영 원칙
- 글마다 같은 이미지를 반복하지 말고, 가능한 한 주제에 맞는 대표 이미지를 따로 사용하세요.
- 비교형 글은 좌우 비교 이미지, 전후형 글은 Before/After 이미지, 우선순위형 글은 2~3분할 비교 이미지를 권장합니다.
- 업로드 용량을 줄이기 위해 WebP 형식과 낮은 품질 압축을 기본으로 사용합니다.
